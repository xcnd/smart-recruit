...
function forceLogout() {
  ...
  if (currentPath !== '/phone-login' && !currentPath.startsWith('/phone-login?')) {
  ...
  // 延迟跳转，给用户阅读通知的时间
  setTimeout(() => {
    const redirect = currentPath !== '/phone-login' ? `?redirect=${encodeURIComponent(currentPath)}` : ''
    window.location.href = `/phone-login${redirect}`
  }, 1500)
}
...
request.interceptors.response.use(
  (response) => {
    ...
    if (res.code === 40901) {
      return Promise.reject(new Error(res.message || 'Duplicate resource'))
    }
