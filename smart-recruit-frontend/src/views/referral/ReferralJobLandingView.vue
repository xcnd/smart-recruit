                ...
                <el-form-item class="jd-form-col-full" prop="resumeUrl">
                  ...
                  >
                    <div class="jd-upload-placeholder" :class="{ 'is-uploading': uploadingResume }">
                      ...
                      <span v-if="!uploadingResume">支持 {{ settingsStore.uploadAllowedExtensions.join(', ') }} 格式</span>
                    </div>
                  </el-upload>
...
async function handleResumeChange(file: UploadFile) {
  ...
  const uploadError = settingsStore.validateUploadFile(rawFile)
  if (uploadError) {
    ElMessage.error(uploadError)
    return
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    const jp = detail.value.jobPosition
