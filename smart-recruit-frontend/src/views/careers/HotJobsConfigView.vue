<template>
  <div class="careers-settings-page">
    <div class="page-header">
      ...
      <el-text type="info" size="small">管理招聘官网「热招职位」页面的展示内容</el-text>
    </div>
      ...
      <div class="settings-content">
          ...
          <el-card shadow="never">
            <template #header>
              <div class="card-header">
                <span class="card-title">页面标题</span>
              </div>
            </template>
        ...
        <template v-if="activeCategory === 'jobs'">
          <el-card shadow="never">
            <template #header>
              <div class="card-header">
                <span class="card-title">热招职位列表</span>
...
import { Briefcase, Check, Plus, Top, Bottom, Delete, Picture, List, Search } from '@element-plus/icons-vue'
...
const fieldToKey: Record<string, string> = {
  title: 'careers_hot_title',
  subtitle: 'careers_hot_subtitle',
  emptyText: 'careers_hot_empty_text',
  filters: 'careers_hot_filters',
}
  ...
  try {
    ...
    for (const config of configs) {
      ...
      if (jsonArrayFields.includes(field)) {
        try { (form as any)[field] = JSON.parse(config.configValue) } catch { }
      } else {
